...
#'
#'- The observed streamflow time series is loaded from the airGR package sample data.
#'- Simulated streamflow time series are created synthetically by perturbing observations 
#'  with multiplicative noise + low-flow-focused perturbations (enforcing non-negativity).
#'- JDKGE is computed and compared with KGE' (Kling et al. (2012)'s version, with ratio 
#'  of coeff. of variation, without the JSD term).
#'
#' Sample data (airGR):
# - Basin: L0123001  
...
if(!requireNamespace("airGR", quietly = TRUE)){ install.packages("airGR") }
if(!requireNamespace("here", quietly = TRUE)){ install.packages("here") }
if(!requireNamespace("hydroGOF", quietly = TRUE)){ install.packages("hydroGOF") }

library(airGR)
library(here)
library(hydroGOF)

#####__Load_functions______________________________________________________#####

cat("Project root detected by here():\n")
print(here())

# Ensure functions are sourced from correct paths (if the repo is at the project root detected by here())
jdkge_path <- here("JDKGE", "R", "jdkge.R")
jsd_path   <- here("JDKGE", "R", "jsd_fd_log.R")

if(!file.exists(jdkge_path) || !file.exists(jsd_path)){
  stop(
    "File paths of 'jdkge.R' and/or 'jsd_fd_log.R' not correctly defined. Please check.\n",
    "Detected project root: ", here(), "\n",
    "Expected paths:\n",
    "  jdkge.R: ", jdkge_path, "\n",
    "  jsd_fd_log.R: ", jsd_path
  )
}
...
# source("/local_path_to_JDKGE/R/jdkge.R")
# source("/local_path_to_JDKGE/R/jsd_fd_log.R")
...
# Perturb observations with multiplicative noise + mild bias
set.seed(123)
...
# Further perturb low flows with additive noise
...
# Plot Qobs (black) vs time
plot(x = dates, y = Qobs_w_gaps,
     type = "l", col = "black",
     xlab = "Date", ylab = "Streamflow (mm/d)",
     main = "JDKGE demo: observed vs simulated",
     las = 1)

# Add Qsim (red)
lines(dates, Qsim_w_gaps, col = "red")

# Legend (top-right)
legend("topright",legend = c("Qobs", "Qsim"),
       col = c("black", "red"),
       lty = 1,
       bty = "n")
...
# Add a text box with JDKGE and KGE' metrics in the hydrograph plot
txt <- sprintf("JDKGE = %.4f\nKGE'  = %.4f", round(jdkge_out$CritValue, 5), round(kge_out$KGE.value, 5))
usr <- par("usr")  # c(x1, x2, y1, y2)
x_left <- usr[1] + 0.02 * (usr[2] - usr[1])
y_top  <- usr[4] - 0.02 * (usr[4] - usr[3])
text(x = x_left, y = y_top, labels = txt, adj = c(0, 1))
